...
namespace DotNet.Utilities
{
    public class FileOperate
    {
		...
		#region 写文件(bin)
		protected void Write_Bin(string FileName, string Content,byte[] SectorBytes, long SectorIndex)
		{
			try
			{
        ...
        #region 读文件(bin)
        protected byte[] Read_Bin(string filename,long SectorIndex)
        {
            if (File.Exists(temp))
            {
                try
        ...
        *****************************************/
        ...
        #region 读取文件内容并写入另一个文件
        public int DataCapy(string sourefile,string targetfile,int souroffset,int targetoffset,int sourelen,int tarfetlen)
        {
        	//打开待读取文件与待写入文件
        	//读取4K数据并判断文件是否读到尾
        	//未读到尾则将数据写入另一文件并刷新缓冲区
        	//读到文件尾则将剩余数据写入目标文件并刷新缓冲区
        	//关闭两个文件
        }
